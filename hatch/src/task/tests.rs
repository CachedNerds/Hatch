...
fn assets_exist(path: &Path) -> bool {
    ...
    path.join("Tupfile").exists() && path.join("config.tup").exists()
        && path.join("Tuprules.tup").exists() && path.join("Tupfile.ini").exists()
        && path.join("test").join("Tupfile").exists()
        && path.join("test/src").join("catch.hpp").exists()
        && path.join("test/src").join("catch.cpp").exists() && file_path.exists()
}
